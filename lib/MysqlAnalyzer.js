    ...
    module.exports = new Class({
        ...
        , listFunctions: function(databaseName) {
            return this.connection.query(`
                SELECT name, type, returns, param_list FROM mysql.proc WHERE db='${databaseName}';
            `).then((fns) => {
                return Promise.resolve(fns.map((record) => {
                    let input = [];
                    let output = [];

                    
                    if (record.type.toLowerCase() === 'function') {
                        
                        // functions
                        record.param_list.toString().split(',').forEach((item) => {
                        ...
                        record.param_list.toString().split(',').forEach((item) => {
                            ...
                            (parts[0].trim() === 'IN' ? input : output).push({
                                  name: parts[1].trim()
                                , type: parts[2].trim()
                            });
                        });
